...
def generate_complaint_pack(
    ...
    barred = [i for i in audit.incidents
              if i.time_barred and i.status in (LATE, NEVER)]
    if barred:
        lines += [
            "",
            "## Older incidents (kept out of this claim)",
            "",
            "The audit also found incidents older than one year. Recovery of "
            "older claims is often contested and the ombudsman window depends "
            "on when the bank was first approached, so they are listed for "
            "your information and NOT included in the demand above:",
            "",
        ] + [
    ...
    lines += [
        "",
        "---",
        "*Generated by RefundRadar, a self-help tool that applies RBI's "
        "published circulars to your own statement. It is not legal advice. "
        "Verify the details above against your records before submitting.*",
    ]
    return "\n".join(lines)
